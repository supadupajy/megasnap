CREATE OR REPLACE FUNCTION public.get_trending_posts(limit_count integer DEFAULT 20)
 RETURNS TABLE(id uuid, content text, image_url text, images text[], location_name text, latitude double precision, longitude double precision, likes bigint, likes_per_hour bigint, views_per_hour bigint, trending_score integer, like_score integer, view_score integer, category text, youtube_url text, video_url text, created_at timestamp with time zone, user_id uuid, display_user_id uuid, user_name text, user_avatar text, hot_since timestamp with time zone, is_seed_data boolean)
 LANGUAGE sql
...
AS $function$
  ...
  candidates AS (
    ...
    WHERE
      ...
      -- 또는 한번이라도 트렌딩에 진입한 적 있는 포스팅 (콜드 스타트 보호)
