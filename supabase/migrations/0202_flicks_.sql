...
CREATE OR REPLACE FUNCTION public.get_flicks_pool(limit_count integer DEFAULT 200)
RETURNS TABLE(
...
AS $$
  ...
  ranked AS (
    SELECT
      vp.*,
      CASE
        WHEN vp.likes > 0 THEN GREATEST(51 - RANK() OVER (ORDER BY vp.likes DESC), 0)::INTEGER
        ELSE 0
      END AS like_rank_score,
      CASE
        WHEN vp.vc_count > 0 THEN GREATEST(51 - RANK() OVER (ORDER BY vp.vc_count DESC), 0)::INTEGER
        ELSE 0
