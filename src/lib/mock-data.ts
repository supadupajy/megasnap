...
export const createMockPosts = (lat: number, lng: number, count: number, userId?: string, bounds?: any): Post[] => {
  return Array.from({ length: count }).map((_, i) => {
    ...
    // 좋아요가 15000 이상이면 인플루언서(diamond/gold) 느낌이 나도록 설정
    let borderType: 'none' | 'silver' | 'gold' | 'diamond' = 'none';
    if (likes > 18000) borderType = 'diamond';
    else if (likes > 15000) borderType = 'gold';
    else if (likes > 10000) borderType = 'silver';

    return {
      id,
      isAd: i % 30 === 0,
      isGif: false,
      isInfluencer: likes > 12000, // 인플루언서 여부 결정
