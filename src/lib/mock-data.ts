...
export const MAJOR_CITIES = [
  {
    ...
    lng: 126.978,
    density: 1000,
    bounds: { sw: { lat: 37.4, lng: 126.7 }, ne: { lat: 37.7, lng: 127.2 } },
  },
  {
    name: "강원/춘천/강릉",
    lat: 37.7511,
    lng: 128.8761,
    density: 500,
    bounds: { sw: { lat: 37.3, lng: 127.9 }, ne: { lat: 38.2, lng: 129.0 } },
  },
  {
    name: "충청/대전/세종",
    lat: 36.3504,
    lng: 127.3845,
    density: 600,
    bounds: { sw: { lat: 36.1, lng: 127.1 }, ne: { lat: 36.7, lng: 127.6 } },
  },
  {
    name: "전라/광주/전주",
    lat: 35.1595,
    lng: 126.8526,
    density: 600,
    bounds: { sw: { lat: 34.8, lng: 126.5 }, ne: { lat: 35.9, lng: 127.2 } },
  },
  {
    name: "경상/대구/구미",
    lat: 35.8714,
    lng: 128.6014,
    density: 700,
    bounds: { sw: { lat: 35.6, lng: 128.3 }, ne: { lat: 36.2, lng: 128.9 } },
  },
  {
    name: "부산/울산/창원",
    lat: 35.1796,
    lng: 129.0756,
    density: 800,
    bounds: { sw: { lat: 35.0, lng: 128.6 }, ne: { lat: 35.6, lng: 129.4 } },
  },
  {
    name: "제주도",
    lat: 33.4996,
    lng: 126.5312,
    density: 500,
    bounds: { sw: { lat: 33.1, lng: 126.2 }, ne: { lat: 33.6, lng: 126.9 } },
  },
] as const;
...
export const AD_COMMENTS = [
  "[AD] 지금 바로 혜택을 확인하세요!",
  "[AD] 당신만을 위한 프리미엄 서비스.",
  "[AD] 신규 오픈! 특별 이벤트 중입니다.",
] as const;

export const FOOD_IMAGES = [10, 15, 20, 25, 30].map(
...
export const getUnsplashUrl = (value: string | number) => {
  if (typeof value === "number") {
    return `https://picsum.photos/id/${(value % 200) + 15}/800/600`;
  }
  return `https://images.unsplash.com/${value}?auto=format&fit=crop&q=80&w=800`;
};
...
const getTierFromId = (id: string): Post["borderType"] => {
  let h = 0;
...
export const createMockPosts = (
  ...
  return Array.from({ length: count }).map((_, i) => {
    const id = specificUserId
      ? `${specificUserId}_post_${i}`
      : Math.random().toString(36).slice(2, 11);
    const isAd = i % 20 === 0;
    const borderType = isAd ? "none" : getTierFromId(id);
    const hasYoutube = !isAd && i % 2 === 0;
    const youtubeId = hasYoutube ? YOUTUBE_IDS_50[i % YOUTUBE_IDS_50.length] : undefined;
    const youtubeUrl = youtubeId ? `https://www.youtube.com/watch?v=${youtubeId}` : undefined;

    const lat = bounds
      ? bounds.sw.lat + randomFn() * (bounds.ne.lat - bounds.sw.lat)
      : centerLat + (randomFn() - 0.5) * 0.1;
    const lng = bounds
      ? bounds.sw.lng + randomFn() * (bounds.ne.lng - bounds.sw.lng)
      : centerLng + (randomFn() - 0.5) * 0.1;

    const image = hasYoutube && youtubeUrl
      ? getYoutubeThumbnail(youtubeUrl) || getUnsplashUrl(UNSPLASH_IDS_100[i % UNSPLASH_IDS_100.length])
      : isAd
        ? FOOD_IMAGES[i % FOOD_IMAGES.length]
        : getUnsplashUrl(UNSPLASH_IDS_100[i % UNSPLASH_IDS_100.length]);

    return {
      id,
      isAd,
      isGif: false,
      isInfluencer: !isAd && ["silver", "gold", "diamond"].includes(borderType || "none"),
      category: (["food", "accident", "place", "animal"] as const)[i % 4],
      user: isAd
        ? {
            id: "ad_partner",
            name: "공식 파트너",
...
export const MOCK_STORIES = MOCK_USERS.map((user) => ({
  id: user.id,
...
export const UNSPLASH_IDS = [...UNSPLASH_IDS_100];
