...
export const sanitizeYoutubeMedia = async <T extends { youtube_url?: string | null; image_url?: string | null; id?: any; images?: any }>(item: T): Promise<T> => {
  // [ULTIMATE FIX] 'Post content 1' 등 텍스트 데이터가 이미지로 인식되는 것을 완전히 차단
  const isValidUrl = (url: any) => {
    if (typeof url !== 'string') return false;
    const clean = url.trim();
    // URL이 http로 시작하고, 'Post content'라는 텍스트가 포함되지 않아야 함 (대소문자 무시)
  ...
  // 1. image_url 강제 검증 및 교체
  if (!isValidUrl(sanitizedItem.image_url)) {
    console.log(`[Sanitize] Invalid image_url detected: ${sanitizedItem.image_url}. Replacing with fallback.`);
    sanitizedItem.image_url = SAFE_FALLBACK;
  }

  // 2. images 배열 강제 검증 및 교체
