...
async function validateYoutube(videoId: string): Promise<boolean> {
  try {
    ...
    // oEmbed 응답이 확실히 200 OK 여야 하며, 제목(title) 데이터가 있어야 합니다.
...
export const seedGlobalPosts = async (currentUserId: string, currentNickname: string, currentAvatar: string) => {
  const confirmClear = confirm("전국구 분포 및 '고성능 유튜브' 시딩을 시작할까요? (기존 중복 데이터는 모두 삭제됩니다)");
  ...
  try {
    ...
    alert(`✨ 완료! 전국 ${globalCount}개 포스팅(유튜브 포함) 생성 완료!`);
  } catch (err) {
    console.error(err);
  }
};
