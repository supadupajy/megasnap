...
// 상세 팝업에서 필요한 모든 정보를 포함하도록 데이터 구조 개선
const MOCK_TRENDING = Array.from({ length: 10 }).map((_, i) => ({
  ...
  location: ["성수동", "제주도", "강남", "강릉", "남산", "광안리", "황리단길", "한옥마을", "송도", "수성못"][i],
  likes: 1200 - (i * 100),
...
const TrendingPosts = ({ isExpanded, onToggle, onPostClick }: TrendingPostsProps) => {
  ...
  const handleItemClick = (e: React.MouseEvent, post: any) => {
    e.stopPropagation(); // 부모의 onToggle 방지
    onPostClick(post);
  };
