...
// 마커 테두리(4.5px) 바깥쪽으로 충분히 떨어뜨려, ring이 마커 테두리와 겹치지 않고
// "마커 외곽을 감싸는 별도의 띠"처럼 보이도록 padding을 더 키움.
// 계산: 마커 테두리 끝(0~60 박스에서 0/60 위치) → ring stroke 중심을 거기서 약 9px 더 바깥에 둠
// (ring stroke 4 + 외곽선 2 + 여유 3 ≈ 9)
const COUNTDOWN_RING_PADDING = -9; // 음수 = 마커 박스 바깥쪽으로 들이는 픽셀
const COUNTDOWN_RING_BOX = 60;    // viewBox 크기 (마커 inner box 크기와 동일)
const COUNTDOWN_RING_SIZE = COUNTDOWN_RING_BOX - COUNTDOWN_RING_PADDING * 2; // 사각 변 길이 = 78
const COUNTDOWN_RING_R = 29;      // 둥근 모서리 반지름 (확장된 박스에 맞춰 키움: 20 + |pad| = 29)
