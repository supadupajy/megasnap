...
const MapContainer = ({ 
    ...
    const initMap = () => {
      try {
        ...
        kakao.maps.event.addListener(map, 'click', (e: any) => {
          ...
          if (onMapClickRef.current) onMapClickRef.current({ lat: e.latLng.getLat(), lng: e.latLng.getLng() });
        });
    ...
    return `
      <div class="marker-content-wrapper">
        ...
        <div class="${animationClass}">
          ${labelHtml}
          <div class="${borderClass}" style="width: 56px; height: 56px; border-radius: 16px; position: relative; z-index: 2; background: white; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.1); border: 2px solid white;">
            <img src="${displayImage}" style="width: 100%; height: 100%; object-fit: cover; ${isViewed ? 'filter: grayscale(1) brightness(0.7);' : ''}" onerror="this.src='${FALLBACK_IMAGE}'" />
          ...
          ${pinColor ? `<div style="position: absolute; bottom: 4px; left: 50%; transform: translateX(-50%); width: 16px; height: 12px;"><svg width="16" height="12" viewBox="0 0 16 12"><path d="M8 12L0 0H16L8 12Z" fill="${pinColor}"/></svg></div>` : ''}
        </div>
      </div>`;
  };
  ...
  useEffect(() => {
    ...
    posts.forEach(post => {
      ...
      const isAd = post.user_name === '공식 파트너';
      ...
      if (!existing) {
        ...
        content.className = `marker-container ${isHighlighted ? 'highlighted' : ''}`;
        ...
        div.style.setProperty('--marker-scale', scale.toString());
        existing.setZIndex(baseZIndex);
      }
    });
  }, [posts, isMapReady, currentLevel, highlightedPostId, viewedPostIds]);
