...
// 외곽 테두리 바깥쪽으로 링이 돌도록 padding을 음수로 둠 → SVG overflow:visible로 잘리지 않음
// 음수 padding만큼 박스가 viewBox 밖으로 확장되어 마커 바깥에 stroke가 그려진다.
const COUNTDOWN_RING_PADDING = -4; // 음수 = 마커 박스 바깥쪽으로 들이는 픽셀
const COUNTDOWN_RING_BOX = 60;    // viewBox 크기 (마커 inner box 크기와 동일)
const COUNTDOWN_RING_SIZE = COUNTDOWN_RING_BOX - COUNTDOWN_RING_PADDING * 2; // 사각 변 길이 = 68
const COUNTDOWN_RING_R = 24;      // 둥근 모서리 반지름 (확장된 박스에 맞춰 키움: 20 + |pad| = 24)
