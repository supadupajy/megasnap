...
// 커스텀 마커 컴포넌트 (lat, lng 프롭이 있어야 구글 맵 위에 렌더링됩니다)
const Marker = ({ post, isViewed, onClick, lat, lng }: { post: any, isViewed: boolean, onClick: () => void, lat: number, lng: number }) => {
...
const MapContainer = ({ posts, viewedPostIds, onMarkerClick, onMapChange, center }: MapContainerProps) => {
  ...
  return (
    <div className="w-full h-full bg-slate-100">
      <GoogleMapReact
        bootstrapURLKeys={{ key: "" }} // API 키 없이도 개발 모드에서 제한적으로 작동합니다.
