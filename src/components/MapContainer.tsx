...
const MapContainer = ({ 
  ...
  const getMarkerInnerHtml = (post: any, isViewed: boolean) => {
    // 1. 광고 판별 (user_name이 '공식 파트너'인 경우 강제 Ad 처리)
    const isAd = post.is_ad === true || post.isAd === true || post.user_name === '공식 파트너' || post.user?.name === '공식 파트너';
    const isMine = authUser && (post.user_id === authUser.id || post.user?.id === authUser.id || post.user?.id === 'me');
    ...
    return `
      <div class="marker-content-wrapper">
        ...
        <div class="${animationClass}">
          ${labelHtml}
          <div class="${borderClass}" style="width: 56px; height: 56px; border-radius: 16px; position: relative; z-index: 2; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); background-color: white;">
