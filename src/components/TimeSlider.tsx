"use client";

import React from 'react';
import { motion } from 'framer-motion';
import { Clock } from 'lucide-react';
import { cn } from '@/lib/utils';

interface TimeSliderProps {
  value: number;
  onChange: (value: number) => void;
}

const TimeSlider = ({ value, onChange }: TimeSliderProps) => {
  const hours = Array.from({ length: 12 }, (_, i) => i + 1);

  return (
    <div className="fixed right-4 top-1/2 -translate-y-1/2 h-1/3 w-14 flex flex-col items-center z-50 pointer-events-none">
      <div className="pointer-events-auto h-full w-full bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 flex flex-col items-center py-4 gap-4">
        <div className="flex flex-col items-center gap-1">
          <Clock className="w-4 h-4 text-green-500" />
          <span className="text-[10px] font-black text-gray-400 uppercase">Time</span>
        </div>

        <div className="flex-1 w-full px-3 relative flex flex-col items-center">
          {/* Track */}
          <div className="absolute inset-y-0 w-1.5 bg-gray-100 rounded-full left-1/2 -translate-x-1/2" />
          
          {/* Active Track */}
          <motion.div 
            className="absolute bottom-0 w-1.5 bg-green-500 rounded-full left-1/2 -translate-x-1/2 origin-bottom"
            initial={false}
            animate={{ height: `${((value - 1) / 11) * 100}%` }}
          />

          {/* Invisible Input for interaction */}
          <input
            type="range"
            min="1"
            max="12"
            step="1"
            value={value}
            onChange={(e) => onChange(parseInt(e.target.value))}
            className="absolute inset-0 w-full h-full opacity-0 cursor-pointer appearance-none [writing-mode:bt-lr] -rotate-180"
            style={{ WebkitAppearance: 'slider-vertical' } as any}
          />

          {/* Markers */}
          <div className="flex-1 w-full flex flex-col justify-between py-1 z-10 pointer-events-none">
            {hours.reverse().map((h) => (
              <div key={h} className="flex items-center justify-center w-full">
                <div className={cn(
                  "w-1 h-1 rounded-full transition-colors",
                  h <= value ? "bg-green-500" : "bg-gray-300"
                )} />
              </div>
            ))}
          </div>
        </div>

        <div className="flex flex-col items-center">
          <span className="text-lg font-black text-green-600 leading-none">{value}</span>
          <span className="text-[8px] font-bold text-gray-400">HOUR</span>
        </div>
      </div>
    </div>
  );
};

export default TimeSlider;