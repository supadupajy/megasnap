    ...
    >
      ...
      {/* 하단 그라데이션 + 액션 알약 + 정보
          순서 (위에서 아래):
            ...
            3) 본문 */}
      {/* 하단 그라데이션 — 영상의 끝 지점이 시각적으로 명확히 인지되도록
          영상 박스 하단부터 본문 영역까지 깊게 드리워진다.
          - 그라데이션 영역 자체는 영상 박스 하단~본문 끝까지로 충분히 길게(약 200~240px).
          - 영상 박스 위쪽은 건드리지 않고, 영상 박스의 아래 1/3 지점부터 페이드가 시작.
          - 본문/액션 버튼 영역은 거의 풀 블랙으로 깔려 가독성 확보 + "영상이 여기서 끝나는구나" 느낌. */}
      <div
        className="absolute left-0 right-0 z-10 pointer-events-none"
        style={{
          bottom: 0,
          height: `${infoHeight + 200}px`,
          background:
            "linear-gradient(to top, rgba(0,0,0,0.95) 0%, rgba(0,0,0,0.92) 30%, rgba(0,0,0,0.75) 55%, rgba(0,0,0,0.35) 80%, rgba(0,0,0,0) 100%)",
        }}
        aria-hidden
      />
