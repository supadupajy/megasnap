...
const LocationButtonWithTimer: React.FC<LocationButtonWithTimerProps> = ({
  ...
  useEffect(() => {
    ...
    console.log('[LocationButtonWithTimer] debug', {
      ...
      computed: {
        borderRadius: debugInfo.borderRadius,
        borderWidth: debugInfo.borderWidth,
        paddingLeft: debugInfo.paddingLeft,
        paddingRight: debugInfo.paddingRight,
      },
      ring: {
        RING_STROKE,
        RING_PADDING,
        pathRadius_r: geom.r,
        buttonHalfHeight: size.h / 2,
        leftArcCenterX: geom.left + geom.r,
        rightArcCenterX: geom.right - geom.r,
        strokeOuterEdgeToBoxEdge: RING_PADDING - RING_STROKE / 2, // 0이면 정확히 align
      },
    });
    ...
    >
        ...
        >
          ...
          {RING_DEBUG && geom && (
            <g>
              ...
              />
              {/* 좌측 반원 중심점 */}
              <circle cx={geom.left + geom.r} cy={geom.top + geom.r} r={1.2} fill="red" />
              {/* 우측 반원 중심점 */}
              <circle cx={geom.right - geom.r} cy={geom.top + geom.r} r={1.2} fill="red" />
              {/* path 시작점 (상단 중앙) */}
              <circle cx={geom.cx} cy={geom.top} r={1.2} fill="blue" />
            </g>
          )}
        </svg>
      )}
      ...
      {RING_DEBUG && debugInfo && geom && (
        ...
        >
          ...
          <div>btn br: {debugInfo.borderRadius} · bw: {debugInfo.borderWidth}</div>
          <div>ring r: {geom.r.toFixed(2)} (btn r: {(size.h / 2).toFixed(2)})</div>
