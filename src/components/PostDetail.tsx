...
const PostDetail = ({ posts, initialIndex, isOpen, onClose, onViewPost }: PostDetailProps) => {
  ...
  // 드래그 거리에 따라 배경 투명도 실시간 연동 (왼쪽으로 밀수록 투명해짐)
  const backdropOpacity = useTransform(dragX, [-screenWidth / 2, 0], [0, 1]);
  const cardOpacity = useTransform(dragX, [-screenWidth / 3, 0], [0, 1]);
  ...
  const handleDragEnd = (event: any, info: PanInfo) => {
    ...
    const swipeThreshold = 40; // 감도 최적화
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      ...
      <style>{`
        [data-radix-portal] div[data-state="open"] { background-color: transparent !important; }
      `}</style>
      ...
      <DialogContent className="p-0 bg-transparent border-none shadow-none w-screen h-screen max-w-none flex items-center justify-center overflow-hidden outline-none focus:ring-0 z-[100]">
        ...
        <div className="relative w-full h-full flex items-center justify-center pointer-events-none">
          <AnimatePresence initial={false} custom={direction} onExitComplete={() => {
            ...
            {!isClosing && (
              <>
                {/* 커스텀 배경 레이어: 드래그 및 종료 애니메이션과 동기화 */}
                <motion.div
                  key="backdrop"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
