  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent 
        ...
        className={cn(
          "fixed inset-0 z-[100] flex items-center justify-center p-0 bg-black/60 backdrop-blur-sm border-none shadow-none w-full h-full max-w-none overflow-hidden translate-x-0 translate-y-0 left-0 top-0 outline-none",
          "data-[state=open]:animate-none data-[state=closed]:animate-none duration-0" // 모든 Tailwind 애니메이션 제거
        )}
      >
        <div 
        ...
        <div className="relative z-10 w-full h-full flex items-center justify-center pointer-events-none p-4">
          {/* AnimatePresence와 motion.div를 제거하여 즉시 렌더링 */}
          {isOpen && (
            <div 
              key={post.id} 
              onClick={onClose}
