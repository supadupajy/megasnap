...
const PostDetail = ({ posts, initialIndex, isOpen, onClose, onViewPost }: PostDetailProps) => {
  ...
  const cardOpacity = useTransform(dragX, [-screenWidth * 0.5, 0], [0, 1]);
  const backdropOpacity = useTransform(dragX, [-screenWidth * 0.8, 0], [0, 1]);
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      ...
      <DialogContent className="p-0 bg-transparent border-none shadow-none w-screen h-screen max-w-none flex items-center justify-center overflow-hidden outline-none focus:ring-0 z-[100]">
        <AnimatePresence initial={false} custom={direction} onExitComplete={() => {
          ...
          {!isClosing && (
            <>
              {/* 동적 배경 레이어: dragX에 따라 투명도 조절 */}
              <motion.div 
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                style={{ opacity: backdropOpacity }}
                className="absolute inset-0 bg-black/70 pointer-events-none z-0"
              />
              ...
              {/* Progress Indicator */}
              <div className="absolute left-1 top-32 bottom-32 w-1.5 z-[110] flex flex-col items-center pointer-events-none">
