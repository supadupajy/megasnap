...
const MapLevelIndicator = ({
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center rounded-[22px] border border-white/70 bg-white/45 px-3 py-3 shadow-[0_12px_32px_rgba(15,23,42,0.18)] backdrop-blur-2xl">
        <div className="text-sm font-black leading-none text-slate-800">+</div>

        <div className="relative my-2 h-[92px] w-6">
          ...
          <div
            className="absolute left-1/2 h-3.5 w-3.5 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-white shadow-lg transition-[top,background-color] duration-200"
            style={{
              top: `${progress}%`,
          ...
          <div
            className="absolute left-1/2 h-px w-5 -translate-x-1/2 bg-white/90 shadow-sm"
            style={{ top: `${hiddenStart}%` }}
          />
        </div>

        <div className="text-sm font-black leading-none text-slate-700">−</div>
        <div className="mt-1.5 text-[11px] font-black leading-none text-slate-600">{safeLevel}</div>
      </div>

      <div
        className={cn(
          'mx-auto mt-1.5 w-max rounded-full border px-2 py-1 text-[10px] font-black shadow-sm backdrop-blur-xl',
          markerVisible
            ? 'border-emerald-200 bg-emerald-50/90 text-emerald-700'
            : 'border-rose-200 bg-rose-50/90 text-rose-700',
        )}
      >
        {markerVisible ? '마커 표시' : '마커 숨김'}
      </div>
    </div>
  );
};
