...
const PostListOverlay = ({ isOpen, onClose, initialPosts, mapCenter, onDeletePost }: any) => {
  ...
  return createPortal(
    <AnimatePresence>
      {isOpen && (
        <div 
          className="fixed inset-0 z-[1000000]" // 최최상위 선언
          style={{ isolation: 'isolate' }}
        >
          {/* 1. 배경 레이어: Blur를 제거하여 렌더링 버그 원천 차단 */}
          <motion.div
            key="backdrop"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={onClose}
            className="fixed inset-0 bg-black/60" // blur 제거 (대신 농도 조절)
            style={{ zIndex: 1 }}
          />

          {/* 2. 콘텐츠 컨테이너: absolute 대신 fixed 사용, pointer-events로 보호 */}
          <motion.div
            key="content"
            initial={{ y: "100%" }}
            animate={{ y: 0 }}
            ...
            className="fixed inset-0 bg-white shadow-2xl flex flex-col overflow-hidden"
            style={{ 
              zIndex: 9999, // 배경보다 압도적으로 높게 설정
              pointerEvents: 'auto', // 터치 간섭 방지
              transform: 'translateZ(0)' // GPU 레이어 강제 고정
            }}
          >
            {/* 고정 헤더 */}
