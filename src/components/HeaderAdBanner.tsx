...
const HeaderAdBanner = () => {
  ...
  if (loading) {
    return (
      <div className="flex-1 ml-2 mr-4 h-10 bg-gray-100 rounded-xl animate-pulse" />
    );
  }
  ...
  if (slot.isRecruitment) {
    return (
      <motion.div
        ...
        className="flex-1 ml-2 mr-4 h-10 bg-gradient-to-r from-indigo-600 to-violet-600 rounded-xl overflow-hidden relative group cursor-pointer shadow-md"
  ...
  return (
    <motion.div 
      initial={{ opacity: 0, scale: 0.95 }}
      animate={{ opacity: 1, scale: 1 }}
      className="flex-1 ml-2 mr-4 h-10 bg-black rounded-xl overflow-hidden relative group cursor-pointer shadow-md border border-white/10"
